...
function findCaliforniaCafes(str) {
    const cafes = JSON.parse(JSON.stringify([
        {
            "name": "Bazaar Cafe",
    ...
    const places = JSON.parse(JSON.stringify([
        {
    ...
    let results = cafes.filter(item => re.test(item.name))
    results.map(item => {
        let placeData = places.filter(i => i.id == item.place_id)
        if (placeData.length > 0) {
            finalRes = [...finalRes, {
                name: item.name,
                street_no: placeData[0].street_no,
                locality: placeData[0].locality,
                postal_code: placeData[0].postal_code,
                lat: placeData[0].lat,
                long: placeData[0].long
            }]
        }
    })
